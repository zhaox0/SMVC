...
class Self_Attn(nn.Module):
    ...
    def forward(self, x):
        ...
        proj_query = self.query(x)  # 计算Query投影
        proj_key = self.key(x)  # 计算Key投影
        energy = torch.matmul(proj_query.permute(0, 2, 1), proj_key)  # 计算注意力能量
        attention = self.softmax(energy)  # 应用Softmax得到注意力权重
        proj_value = self.value(x)  # 计算Value投影
        out = torch.matmul(proj_value, attention.permute(0, 2, 1))  # 加权求和
        out = out.squeeze(1)  # 压缩维度
        return out, attention  # 返回输出和注意力图
...
class Repara(nn.Module):
    ...
    def __init__(self, dim_emb, hidden_sizes, latent_size):
        ...
        self.linearelu_mu = nn.Linear(latent_size, latent_size)  # 均值投影层
        self.linearelu_logvar = nn.Linear(latent_size, latent_size)  # 对数方差投影层

    def gaussian_param_projection(self, x):
        return self.linearelu_mu(x), self.linearelu_logvar(x)  # 输出均值和方差
    ...
    def forward(self, x):
        x = self.linearelu(x)  # 通过前馈网络
        mu, logvar = self.gaussian_param_projection(x)  # 获取高斯参数
        z = self.reparameterize(mu, logvar)  # 重参数化
        return z, mu, logvar  # 返回潜在变量和参数
...
class ConvNorm(torch.nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1,
        ...
        if padding is None:
            ...
            padding = int(dilation * (kernel_size - 1) / 2)  # 自动计算padding
...
class Decoder(nn.Module):
    ...
    def __init__(self, dim_neck, dim_emb, dim_pre):
        ...
        self.linear_projection = LinearNorm(1024, 80)  # 输出mel频谱
...
class Generator(nn.Module):
    ...
    def __init__(self, dim_neck, dim_emb, dim_pre, freq):
        ...
        self.attn = Self_Attn(dim_emb, dim_emb)  # 自注意力
        self.encoder = Encoder(dim_neck, dim_emb, freq)  # 编码器
        self.decoder = Decoder(dim_neck, 256, dim_pre)  # 解码器
        self.postnet = Postnet()  # 后处理网络
        self.reparam = Repara(dim_emb, hidden_sizes=320, latent_size=256)  # 重参数化模块
